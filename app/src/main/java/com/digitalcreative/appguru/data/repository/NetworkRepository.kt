...
class NetworkRepository @Inject constructor(private val service: ApiService) {
    ...
    suspend fun editClassName(
        ...
        return try {
            val body = MultipartBody.Builder()
                ...
                .addFormDataPart("nama_kelas", className)
                .build()
            val response = service.editClassName(teacherId, classId, body)
            if (response.status == STATUS_SUCCESS) {
                Result.Success(response.message)
            } else {
                Result.ErrorRequest(response.message)
    ...
    suspend fun editAssignment(
        ...
        return try {
            val body = MultipartBody.Builder()
                .setType(MultipartBody.FORM)
                .addFormDataPart("nama_tugas", assignmentTitle)
                .addFormDataPart("deskripsi_tugas", assignmentDescription)
                .build()
            val response = service.editAssignment(teacherId, classId, assignmentId, body)
            if (response.status == STATUS_SUCCESS) {
                Result.Success(response.message)
            } else {
                Result.ErrorRequest(response.message)
            ...
            Log.e("NetworkRepository", "EditAssignment -> ${e.localizedMessage}")
            Result.ErrorRequest(CONNECTION_ERROR)
        } catch (e: Exception) {
            Log.e("NetworkRepository", "EditAssignment -> ${e.localizedMessage}")
            Result.ErrorRequest(UNKNOWN_ERROR)
    ...
    suspend fun editSection(
        ...
        return try {
            val body = MultipartBody.Builder()
                ...
                .addFormDataPart("grup_soal", section)
                .build()
            val response = service.editSection(teacherId, classId, assignmentId, sectionId, body)
            if (response.status == STATUS_SUCCESS) {
                Result.Success(response.message)
            } else {
                Result.ErrorRequest(response.message)
            ...
            Log.e("NetworkRepository", "EditSection -> ${e.localizedMessage}")
            Result.ErrorRequest(CONNECTION_ERROR)
        } catch (e: Exception) {
            Log.e("NetworkRepository", "EditSection -> ${e.localizedMessage}")
            Result.ErrorRequest(UNKNOWN_ERROR)
    ...
    suspend fun editQuestion(
        ...
        return try {
            val body = MultipartBody.Builder()
                .setType(MultipartBody.FORM)
                .addFormDataPart("soal", formData["soal"] ?: "")
                .addFormDataPart("id_pilihan_jawaban", "[1, 2]")
                .addFormDataPart("bobot_pilihan_jawaban", formData["bobot_pilihan"] ?: "")
                .build()
            val response =
                service.editQuestion(teacherId, classId, assignmentId, sectionId, questionId, body)
            if (response.status == STATUS_SUCCESS) {
                Result.Success(response.message)
            } else {
                Result.ErrorRequest(response.message)
            ...
            Log.e("NetworkRepository", "EditQuestion -> ${e.localizedMessage}")
            Result.ErrorRequest(CONNECTION_ERROR)
        } catch (e: Exception) {
            Log.e("NetworkRepository", "EditQuestion -> ${e.localizedMessage}")
            Result.ErrorRequest(UNKNOWN_ERROR)
